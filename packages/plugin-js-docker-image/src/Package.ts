...
export default class Package extends AbstractPackage {
    ...
    protected buildMakefile(vars: any): MakefileTemplate {
        return new MakefileTemplate({makefile: false !== vars.makefile, ...(vars.makefile || {})})
            ...
            .addPredefinedTarget('build-image', 'docker-build', {sourceLocalEnvLocal: true, awsProfile: true, awsEcrLogin: true, tag: '$(ecr_url)/$(image_tag)', path: vars.image_dir || '.', buildArgs: vars.image_buildArgs || {}})
            .addMetaTarget('build', ['build-code', 'build-image'])
            .addMetaTarget('install', ['install-code'])
            .addPredefinedTarget('push', 'docker-push', {sourceLocalEnvLocal: true, awsProfile: true, awsEcrLogin: true, tag: '$(ecr_url)/$(image_tag)'})
